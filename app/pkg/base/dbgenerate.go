...
import (
	"fmt"
	"github.com/qinguoyi/asyncjob/app/pkg/repo"
...
func NewUidGroup() (map[string]*Uid, error) {
	if uidGroup != nil {
		...
		once.Do(func() {
			...
			uidInfo, err := repo.NewUidRepo().GetAllBusinessID(lgDB)
			if err != nil {
				panic(err)
			}
			for _, uid := range uidInfo {
				newUidN, err := newUid(uid.BusinessId)
				if err != nil {
					panic(err)
				}
				uidGroup[uid.BusinessId] = newUidN
			}
		})
	}
	return uidGroup, nil
...
func (u *Uid) NextId() (int64, error) {
	...
	case <-time.After(time.Second):
		return 0, fmt.Errorf("get uid timeout")
	case uid := <-u.ch:
		return uid, nil
	}
}
